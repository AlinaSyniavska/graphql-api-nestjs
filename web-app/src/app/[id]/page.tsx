...
function SingleMovie({ params }: { params: { id: string } }) {
  ...
  return (
    <NextUIProvider>
      <section
        className={
          'h-screen bg-gradient-to-r from-purple-500 to-pink-500 flex items-center justify-center flex-col'
        }
      >
            ...
            <div className="flex flex-col col-span-6 md:col-span-8">
              <div className="flex justify-between items-start">
                <div className="flex flex-col gap-0">
                  ...
                  <ul className={'mt-5 text-small text-foreground/80'}>
                    Comments:
                    {data?.getMovieById.movieComment.map((item) => (
                      <li key={item.id} className={'text-xs my-2'}>
                        {item.description}
                      </li>
                    ))}
                  </ul>
